...
[Exemple physique]{.underline}: loi d'un tirage pile / face, $p$: biais de la pièce.

::: {#exm-binomiale}

## Loi binomiale

 $\mathbb{P}(X=k) = \binom{n}{k} p^k (1-p)^{n-k}$, pour $k \in E=\{0,\dots,n\}$, qui modélise le nombre de succès parmi $n$ lancers.
:::

[Exemple physique]{.underline}: nombre de tirages de faces avec un bias $p$ de la pièce.

En sommant des variables aléatoires indépendantes de *loi de Bernoulli* on obtient une loi binomiale :
