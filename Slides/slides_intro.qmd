...
fig1.update_layout(height=720, width=1280)
...
Le nom Monte-Carlo: venu du besoin de confidentialité du projet
...
- Popularisation croissante:
  ...
  - Essor des méthodes de calcul parallèle (GPU, cluster, etc.)
...
- Domaine principaux impactés:
  ...
  - apprentissage automatique: utilisation de l'aléatoire pour généré des scénarios (e.g., AlphaGo, AlphaGeometry)

Exemple d'AlphaGo (et autres jeux):

Recherche arborescente Monte-Carlo: analyse des mouvements les plus prometteurs, en élargissant l'arbre de recherche sur la base d'un échantillonnage aléatoire de l'espace de recherche
