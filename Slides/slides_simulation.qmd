...
Supposons disposer d'une autre densité $g$ (**loi des propositions**) t.q. :
...
[Remarque]{.underline}: $m \geq 1$ car
$$
...
- $r(x) =f(x) / (m\cdot g(x)) = x^3$, pour $x \in [0,1]$. On simule donc $(Y_1, U_1)$ et on teste si $4 U_1 \leq 4 Y_1^3$, etc.
...
[Loi cible]{.underline}: loi uniforme sur le disque unité, $f(x)\triangleq{1\hspace{-3.8pt} 1}_{x_1^2+x_2^2 \leq 1}(x)$ pour $x=(x_1,x_2)\in\mathbb{R}^2$

<br>

[Loi majorante]{.underline}: loi uniforme sur le carré $[-1,1]^2$, $g(x)\triangleq{1\hspace{-3.8pt} 1}_{[-1, 1]^2}(x)$ et $m=1$
...
[Loi majorante]{.underline}: loi uniforme sur le rectangle $[-2,3]\times [-1.5,1.5]$, $f(x)\triangleq{1\hspace{-3.8pt} 1}_{[-2,3]\times [-1.5,1.5]}(x)$ et $m=1$
...
Supposons $A\subset B \subset \mathbb{R}^d$, deux ensembles mesureables pour la mesure de Lebesgue.
Pour générer selon une loi uniforme sur $A$, connaissant un générateur uniforme sur $B$, on peut utiliser la méthode du rejet, en tirant $Y_i \sim \mathcal{U}(B)$ (i.i.d) et en ne gardant  $Y_i$ que si $Y_i \in A$.
:::
...
[Loi de Bernouilli]{.underline}:  avec $U_1, \ldots, U_n$ i.i.d uniformes sur $[0,1]$ (méthode d'inversion):
$$
...
C'est la méthode utilisée par `numpy.random.poisson`,
cf. [code source (GitHub)](https://github.com/numpy/numpy/blob/main/numpy/random/src/distributions/distributions.c#L540) (en passant à l'exponentiel), qui fut proposée par D. Knuth; Source: [Wikipedia](https://en.wikipedia.org/wiki/Poisson_distribution#cite_note-Knuth1997-68)
...
\end{align*}
$$
Le lemme précédent donne $\mathbb{P}(S_n \leq 1) = 1 - \sum_{n=0}^{n-1} e^{-\lambda} \dfrac{\lambda^k}{k!}$
et
