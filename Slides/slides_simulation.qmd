...
[Question: Comment simuler en pratique des variables aléatoires i.i.d?]{.r-fit-text}
...
[Approche: commencer par les v.a. uniformes et en déduire les autres lois]{.r-fit-text}
<br>
<br>
...
fig = go.Figure(layout=go.Layout(autosize=True, width=1000, height=720))
...
- Les nombres flottant entre $0$ et $1$ donnés par la machine sont de la forme $k/2^p$, pour $k \in \{0, \ldots, 2^{p-1}\} \implies$ impossibilité de générer certains nombres.
...
[Remarque]{.underline}: ces nombres sont obtenus depuis des **nombres entiers** générés aléatoirement et uniformément sur grand interval, puis une transformation simple (normalisation) permet d'obtenir des nombres flottants (&#127468;&#127463;: *floats*) entre 0 et 1.
...
1. Initialisation: une graine (&#127468;&#127463;: *seed*) $U_0$, détermine la première valeur (choix arbitraire)
...
[Cas $\subset$:]{.underline}

Soit $x \in \mathbb{R}$ t.q. $F(x) \geq q$, alors par définition de l'inf dans @eq-inf, $x \geq F^\leftarrow(q)$

<br>

. . .

[Cas $\supset$:]{.underline}

Soit  $x \in \mathbb{R}$ t.q. $x \geq F^\leftarrow(q)$ alors pour tout $\epsilon > 0$, $x + \epsilon > F^\leftarrow(q)$, donc $F(x + \epsilon) \geq q$; par continuité à droite de $F$, $F(x) \geq q$
...
$$
\forall x\in\mathbb{R}, \quad \mathbb{P}(F_X^\leftarrow(U) \leq x) = \mathbb{P}(U \leq F_X(x)) 
$$
...
Supposons disposer d'une autre densité $g$ t.q. :
...
[Remarque]{.underline}: $m \leq 1$ car
$$
	1 = \int_\mathbb{R} f(x) \,  dx \class{fragment}{{}\leq m \int_\mathbb{R} g(x)\,  dx} \class{fragment}{{}= m}
$$
...
Pour simuler $X$ de densité $f$, simuler $Y_n$ de densité $g$ et $U_n$ uniforme et puis  vérifier si $U_n \leq r(Y_n)$
...
- $r(x) =f(x) / (mg(x)) = x^3$, pour $x \in [0,1]$. On simule donc $(Y_1, U_1)$ et on teste si $4 U_1 \leq 4 Y_1^3$, etc.
...
- $f(x) = 4x^3 {1\hspace{-3.8pt} 1}_{[0,1]}(x)$
- $f(x)$ est majorée par $4 x \cdot {1\hspace{-3.8pt} 1}_{[0,1]}(x)$ $\implies$ $g = \tfrac{x}{2} \cdot {1\hspace{-3.8pt} 1}_{[0,1]}$ et $m=8$ conviennent
- $r(x)=f(x) / (mg(x)) = x^3$, pour $x \in [0,1]$.
...
- il existe $m > 0$ tel que $f(x) \leq m \cdot g(x)$ et $r(x) = \frac{f(x)}{mg(x)}$
...
Pour $x \in \mathbb{R}$ et $n \in \mathbb{N}^{*}$, on écrit
$\mathbb{P}(X \leq x, T=n)= \mathbb{P}(U_1 > r(Y_1), \dots, U_{n-1} > r(Y_{n-1}), U_n \leq r(Y_n), Y_n \leq x)$


::: {.fragment fragment-index=1}
$$
    \mathbb{P}(X \leq x, T=n) = {\color{blue}\mathbb{P}(U_1 > r(Y_1))}^{n-1} \cdot {\color{brown}\mathbb{P}(U_n \leq r(Y_n), Y_n \leq x)}, \quad \text{( tirages i.i.d.)}
$$
:::
...
\begin{align*}
    ...
    & \class{fragment}{{} = \int_\mathbb{R} \left( \int_0^1  {1\hspace{-3.8pt} 1}_{\{u \leq r(y)\}} \,  du\right) {1\hspace{-3.8pt} 1}_{\{y \leq x\}}  g(y)\,  d y} \\
		...
		& \class{fragment}{{} = \int_{-\infty}^x \dfrac{f(y)}{m}\,  d y } \\
 		& \class{fragment}{{} = \dfrac{F(x)}{m} , \quad F \text{ fonction de répartition associée à} f}
\end{align*}
}
$$
...
[Loi cible]{.underline}: loi uniforme sur le disque unité, $f(x)\triangleq{1\hspace{-3.8pt} 1}_{x_1+x_2^2 \leq 1}(x)$ pour $x=(x_1,x_2)\in\mathbb{R}^2$

<br>

[Loi majorante]{.underline}: loi uniforme sur le carré $[-1,1]^2$, $f(x)\triangleq{1\hspace{-3.8pt} 1}_{[-1, 1]^2}(x)$  ($m=1$ fonctionne).
...
[Loi majorante]{.underline}: loi uniforme sur le rectangle $[-2,3]\times [-1.5,1.5]$, $f(x)\triangleq{1\hspace{-3.8pt} 1}_{[-2,3]\times [-1.5,1.5]}(x)$  ($m=1$ fonctionne).
...
//| echo: false
//| layout-ncol: 1
md`Ratio estimée: **${ratio_cardioid}**`
md`Aire (bleue) estimé: **${aire_cardioid}**`
```
...
Suppons $A\subset B \subset \mathbb{R}^d$ pour générer selon une loi uniforme sur $A$, connaissant un générateur uniforme sur $B$, on peut utiliser la méthode du rejet, en tirant $Y_i \sim \mathcal{U}(B)$ (i.i.d) et en ne gardant  $Y_i$ que si $Y_i \in A$.
:::
...
<!-- On peut donc appliquer la méthode de rejet sans la connaissance de $|A|$! -->