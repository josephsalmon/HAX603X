...
def marche(x, N):
    ...
    # Calculate the cumulative sum of steps and add to initial value
...
plt.figure()
for s in S:
    plt.plot(absc, s, marker='o', markersize=3, linestyle='-')
...
4. Tracer quelques trajectoires du jeu pour $x=10$, $a=20$, $b=0$.
...
for i in range(num_trajectories):
    M = ruine(10, 20, 0)
    plt.plot(np.arange(len(M)), M, marker='o', linestyle='-')
...
On voit dans chaque graphique que les premiers $10\%$ de la trajectoire correspondent à la trajectoire précédente compressée (zones grisées). Le caractère discret est bien visible sur les deux premières trajectoires, puis on le le distingue plus à l'oeil sur les suivantes.
