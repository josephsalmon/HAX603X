...
`np.sort()`, `np.argsort()`, `np.partition()`, `np.argpartition()` on pourra consulter l'aide en ligne pour plus de détails: [https://numpy.org/doc/stable/reference/routines.sort.html](https://numpy.org/doc/stable/reference/routines.sort.html), et les teser sur le vecteur `v2` par exemple.
...
On peut aussi extraire des sous parties de vecteurs, par exemple `x[3:7]` pour extraire les valeurs d'indice $3$, $4$, $5$ et $6$, ou bien `x[3:]` pour extraire les valeurs d'indice $3$, $4$, $5$, etc. jusqu'à la fin. Enfin, on peut extraire des sous suites d'indices pair ou impair, par exemple `x[::2]` pour extraire les valeurs d'indice pair, ou bien `x[1::2]` pour extraire les valeurs d'indice impair.
