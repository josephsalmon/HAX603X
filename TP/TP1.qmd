...
`numpy` est l'outils de base en Python pour faire du calcul vectoriel et matriciel.
...
`np.sum()`, `np.prod()`, `np.len()`, `np.min()`, `np.max()`, `np.nanmax()`, `np.argmax()`, `np.mean()`, `np.median()`, `np.var()`, `np.std()`.



Enfin on peut aussi utiliser des fonctions de tri, partiel ou non:
`np.sort()`, `np.argsort()`, `np.partition()`, `np.argpartition()` on pourra consulter l'aide en ligne pour plus de détails: [https://numpy.org/doc/stable/reference/routines.sort.html](https://numpy.org/doc/stable/reference/routines.sort.html)


#### Question : Opérations sur les vecteurs

Créez un vecteur de taille 5 et appliquez-lui les fonctions précédentes. Si vous ne comprenez pas la sortie, utiliser l'aide avec `??` ou `?`.
...
Exécutez les commandes suivantes et essayer de analysez les sorties :
```{python}
# create a vector starting from 3 finishing at -8 with step 1, but in a decreasing order
np.arange(9, 13)
...
Enfin, pour extraire la valeur $i$ d'un vecteur `x`, on tapera `x[i]` (avec la convention que Python commence à énumérer à 0).
Plus généralement, pour extraire les $3\ieme$, $4\ieme$ et $7\ieme$ composante on tapera `x[[3,4,7]]`. Le vérifier sur un vecteur de taille 10.
...
Créer une matrice de taille $4\times 5$ dont les entrées sont $i.i.d$ de loi de Laplace d'espérance 0 et de variance 2. Lancer plusieurs fois le code et observez les changements. On pourra s'aider [de l'aide en ligne](https://numpy.org/doc/stable/reference/random/generator.html) si besoin.
