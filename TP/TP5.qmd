...
On note $B_d$ la boule unité en dimension $d$ et $V_d$ son volume.
Ainsi $B_d=\{x\in \mathbb R^d, \sum_{i=1}^d x_i^2 \leq 1\}$.
En notant $f(\cdot) = \1_{B_d}(\cdot)$ on peut donc écrire $V_d = \int_{\mathbb{R}^d} f(x)dx$.

On souhaite estimer $V_d$ pour de grande valeurs de $d$.

Pour rappel un calcul exact donne $V_d = \frac{\pi^{d/2}}{\Gamma(d/2+1)}$.

1. Implémentez la méthode de Riemann pour calculer $V_d$ pour $d=1,\dots, 100$.
...
def rieman(n, d): 
    ...
    X = np.meshgrid(*[X]*d)  # Create a grid of points in the range [-1, 1]
    X = np.array([x.flatten() for x in X]).T
    Y = np.array([f(x) for x in X])* 1.
    dx = (2 / (n_discr))**d
    return np.sum(Y) * dx, n_eff
```



1. Implémentez la méthode de Monte Carlo standard pour ce même calcul. Que constatez-vous ? Pourquoi ?
...
for d in range(2, d_max):
    ...
    in_r, n_eff = rieman(n, d)
    # use same number of evaluations for both methods
    # to make a fair comparison, at least > n
    in_mc, ic_mc = montecarlo(n_eff)
    # print(d, n_eff)

    # print("Riemann:     ", in_r)
    # print("Monte Carlo: ", in_mc)
    # print("True Volume: ", true_volume(d))
