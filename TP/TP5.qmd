...
Notont $a=(a_1,\dots,a_d)^\top$ et $b=(b_1,\dots,b_d)^\top$ deux points de $\mathbb{R}^d$.
...
Concernant la méthode de Monte-Carlo l'esimtateur est donnée par:
$$
...
n_list = np.logspace(2, 6, 15)

n_init = int(n_list[0])
in_mc, _, _ = montecarlo(n_init, d)
rel_err_mc_init = np.abs(in_mc - true_volume(d)) / true_volume(d)
in_r, _, _ = riemann(int(n_init**(1/d)), d)
rel_err_r_init = np.abs(in_r - true_volume(d)) / true_volume(d)

n_list_r = np.zeros_like(n_list)
n_list_r[0] = int(n_init**(1/d))**d

for idx, i in enumerate(n_list[1::]):
    n = int(i)
    n_r = int(n**(1/d))
    n_list_r[idx+1] = n_r ** d
    in_r, n_eff, X  = riemann(n_list_r[idx+1], d)
    in_mc, _, _ = montecarlo(n, d)
    rel_err_mc = np.abs(in_mc - true_volume(d)) / true_volume(d)
    rel_err_r = np.abs(in_r - true_volume(d)) / true_volume(d)

    ax.plot(n, rel_err_mc, marker="s",color='peru')
    ax.plot(n, rel_err_r, marker="s",color='dodgerblue')

ax.set_yscale('log')
ax.set_xscale('log')

ax.plot(n_list, rel_err_mc_init * (n_init **(1/2))/(n_list**(1/2)), label=r"$\propto n^{-\frac{1}{2}}}$", color='peru', linestyle='--')

ax.plot(n_list_r, rel_err_r_init * (n_list_r[0] **(1/d)) /(n_list_r**(1/d)), label=r"$\propto n^{-\frac{1}{d}}}$", color='dodgerblue', linestyle='--')

plt.title(f"Erreur relative en fonction du nombre d'évaluations pour $d={d}$")

ax.plot(n_init, rel_err_mc_init, marker="s", label="Monte-Carlo", color='peru')
ax.plot(int(n_init**(1/d))**d, rel_err_r_init, marker="s", label="Riemann", color='dodgerblue')
