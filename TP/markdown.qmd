...
* pour créer des sites web, des rapports et des présentations ; voir Combinaison de code et de texte dans Quarto Markdown pour plus d'informations
* pour les répertoires de logiciels ou de réplications d'articles de recherche
...
* Les fichiers markdown peuvent être ouverts à l'aide de n'importe quel éditeur de texte brut
* Markdown est indépendant du système d'exploitation
* Markdown est très lisible, même lorsqu'il n'est pas rendu
* De nombreux sites web prennent en charge la syntaxe markdown, par exemple GitHub (appelé GitHub-flavoured markdown) et Reddit.
...
mais vous ne voudrez presque jamais les écrire vous-même ! En pratique, il est plus facile d'exporter un fichier markdown à partir d'un dataframe **pandas** en utilisant `df.to_markdown()` ou d'utiliser le site web pratique, [générateur de tableaux markdown](https://www.tablesgenerator.com/markdown_tables).
...
```markdown
- [x] Terminer le chapitre 1
- [ ] Éditer le chapitre 2
- [ ] Lancer le livre :rocket:
```
...
Des notes de bas de page peuvent être créées en utilisant `[^1]` suivi de `[^2]`, et ainsi de suite, ou en utilisant des notes de bas de page liées au contenu comme `[^note]`. Voici ces trois utilisations : un exemple[^1], et un autre[^2], tandis que le troisième se trouve ici[^note] et a une étiquette à la place d'un numéro (que vous ne pouvez pas voir lorsqu'elle est rendue). Vous devrez faire défiler jusqu'au bas de la page pour voir les informations associées à ces notes de bas de page, mais la syntaxe pour remplir leurs informations est :

```markdown
[^1]: Première note de bas de page.
[^2]: Chaque nouvelle ligne dans une note de bas de page doit être préfixée par 2 espaces.
