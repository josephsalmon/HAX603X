...
Il existe également de nombreuses méthodes pour l'intégration numérique déterministe (méthodes des rectangles, trapèzes,...). Ces méthodes sont très rapides et efficaces en petite dimension, mais elles le le sont plus du tout ne le sont plus du tout pour des dimensions plus grandes ou pour l'intégration de fonction peu régulières. Ainsi une {intégration par Monte Carlo} pour le cas de la grande dimension sera bien plus profitable.
...
print("Gagne en changeant : ", gagne_avec_change / NbSimulations * 100, "%")
print("Gagne sans changer : ", gagne_avec_conserve / NbSimulations * 100, "%")
...
$$
où $F^{-1}_{\cN(0,1)}$ est la fonction quantile\footnote{c'est-à-dire l'inverse de la fonction de répartition} de la loi normale centrée réduite et 
$$
...
print([m - 1.96 * np.sqrt(s / n), m, m + 1.96 * np.sqrt(s / n)])
```
...
m1 = np.mean(gX)
s1 = np.var(gX)
confidence_interval1 = [m1 - 1.96 * np.sqrt(s1 / n), m1, m1 + 1.96 * np.sqrt(s1 / n)]
print("Direct method confidence interval:", confidence_interval1)

## Control variable
ghX = np.exp(X**2) - 1 - X**2
m2 = np.mean(ghX) + 4/3
s2 = np.var(ghX)
confidence_interval2 = [m2 - 1.96 * np.sqrt(s2 / n), m2, m2 + 1.96 * np.sqrt(s2 / n)]
print("Control variable confidence interval:", confidence_interval2)
...
$$
	\var\left(\frac{g(X)+g(h(X))}{2}\right)  \leq \frac{ \var (g(X))+\var(g(h(X))}{4}=\frac{\var (g(X))}{2},
$$
...
$$
	\var\left(\frac{g(X)+g(h(X))}{2}\right) \leq 2\frac{\var (g(X))+\var(g(h(X))}{4}=\var (g(X)),
$$
...
m3 = np.mean(gaX)
s3 = np.var(gaX)
confidence_interval3 = [m3 - 1.96 * np.sqrt(s3 / n), m3, m3 + 1.96 * np.sqrt(s3 / n)]
print("Antithetic variable method confidence interval:", confidence_interval3)
```
...
$$
I=\mathbb E[g(X)]=\int g(x)f(x)dx=\int \frac{g(x)f(x)}{h(x)}h(x)dx=\mathbb{E}\left[ \frac{g(Y)f(Y)}{h(Y)}\right].
$$
On a gagné quelque chose si $\var\Big(\frac{gf}{h}(Y)\Big)<\var\big(g(X)\big)$. \\

Comme pour la méthode de la variable de contrôle, on souhaite que la nouvelle variable $\frac{g(Y)f(Y)}{h(Y)}$ soit de variance aussi petite que possible, donc proche d'une constante. Si on prend $h(x)=\frac{g(x)f(x)}{\mathbb E[g(X)]}$ (qui est bien une densité si $g$ est positive), alors la variance de $\frac{g(Y)f(Y)}{h(Y)}$ est nulle. Evidemment, ce résultat n'est pas utilisable en pratique puisqu'on cherche justement à calculer $\mathbb E[g(X)]$. Cependant, il guide la recherche d'une bonne fonction $h$ vers une approximation de $|gf|$ normalisée pour obtenir une densité.
...
m4 = np.mean(gpX)
s4 = np.var(gpX)
confidence_interval4 = [m4 - 1.96 * np.sqrt(s4 / n), m4, m4 + 1.96 * np.sqrt(s4 / n)]
print("Importance sampling confidence interval:", confidence_interval4)
