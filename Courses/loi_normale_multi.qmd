...
En particulier chaque (loi marginale) $X_j$ suit alors une loi gaussienne (choisir ci-dessus $t = e_j$,  les autres égaux à $0$).
...
Commençons par le cas $\mu=0$ et $\Sigma = \mathrm{Id}_d$ correspond à la loi gaussienne centrée réduite, notée $\mathcal{N}(0, \mathrm{Id}_d)$.
...
$$
\varphi_{0,\mathrm{Ids}_d}(x) = \frac{1}{ \sqrt{(2\pi)^d}} \exp\left( -\tfrac{1}{2}x^\top x   \right) \enspace.
$$

Dans le cas général, $\mu \in \mathbb{R}^d$ et $\Sigma \in \mathbb{R}^{d \times d}$ (symétrique et définie positive) sont donnés, et on note
$$
X \sim \mathcal{N}(\mu,\Sigma), \quad \forall x \in \mathbb{R}^d
$$
