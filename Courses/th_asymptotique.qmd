    ...
    ui.row(
        ...
        ui.column(
            ...
            ui.input_select(
                ...
                {'uniforme': 'Uniforme', 'sinusoidale' : 'Sinusoidale'},
            ),
        ),
    ),
)
...
y[mask == 1] = 1
var = np.sum(y * x**2 *((x_max - x_min) / nnzeros))
...
# y_display = convolve(y, 1, (x_max - x_min) / nnzeros)
...
def server(input, output, session):
    ...
    fig.update_xaxes(range=[-2, 2], position=0.)
    fig.update_yaxes(range=[0, 1 / np.sqrt(2 * var * np.pi)*1.5], position=0.5, showticklabels=False)
    ...
    def _():
        if str(input.loi()) == 'uniforme':
            ...
            # else str(input.loi()) == 'sinusoidale':
            y = np.zeros(nnzeros)
            mask = np.where(np.abs(x) >= 0.5, 1, 0)
            y[mask == 1] = 0
            y = np.sin(np.pi*x)+1
            y = y / (np.sum(y) * ((x_max - x_min) / nnzeros))

        var = np.sum(y * x**2 *((x_max - x_min) / nnzeros)) - (np.sum(y * x *((x_max - x_min) / nnzeros)))**2


        y_display = convolve(y, input.n_iter(), (x_max - x_min) / nnzeros)
        # Update data in fig:
        ...
        fig.update_yaxes(range=[0, 1.5 / np.sqrt(2 * var * np.pi)], position=0.5, showticklabels=False)
