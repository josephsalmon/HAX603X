...
app_ui = ui.page_fluid(
    ...
    ui.row(
        ui.column(6,
            ui.input_slider("p", "Paramètre de Bernoulli", 0.01, 0.99, value=0.5, step=0.01)
        ),
        # ui.column(4, ui.input_slider("step", "n", 1, n_samples, value=10)),
        ui.column(6, ui.input_slider("n_samples", "Nombres de tirage total", 10, 1000, value=30, step=10)
        )
    )
)
...
def server(input, output, session):
    ...
    def my_widget():
        ...
        fig = make_subplots(
                    ...
                    subplot_titles=(
                        "Moyenne empirique en fonction du nombre de tirages <br>(loi de Bernoulli)",
                        "Tirages aléatoires (rouge:1 , bleu:0)",
                    ),
                    row_heights=[8, 1],
                )
        ...
        #             marker=dict(size=20),
        #             x=[iterations[step-1]],
        #             y=[means_samples[step-1]],
        #             showlegend=False
        #         ), row=1, col=1,
        # )

        fig.add_trace(
                go.Heatmap( x=iterations+0.5,
                            z=[samples],
        ...
        fig.update_xaxes(range=[0, n_samples + 1])
