...
app_ui = ui.page_fluid(
    ...
    ui.row(
        ...
        ui.column(4,
            ui.input_slider("n_repetitions", "Nombre de répétitions", 2, 1000, value=200, step=1)
        ),
        # ui.column(4, ui.input_slider("step", "n", 1, n_samples, value=10)),
        ui.column(4, ui.input_slider("n_samples", "Nombre de tirages aléatoires ", 2, 500, value=100, step=1)
        )
    )
)
...
def server(input, output, session):
    ...
    def my_widget():
        ...
        fig.add_trace(
            go.Histogram(y=means_samples[:,-1],
                         opacity=0.75,
                         marker_color = 'black',
                         xbins = dict(start=0,
                                 end=1,
                                 size=1/int(np.sqrt(n_repetitions))),
        ...
        fig.update_xaxes(range=[0, 1.1 / np.sqrt(2*np.pi* p*(1-p) / n_samples)], row=1, col=2)
        ...
        for trace in fig['data']:
            ...
            # template="simple_white",
            # showlegend=True,sur plusieurs répétitions et histogrammes après"+ str(n_samples)"+ tirages" + "(loi de Bernoulli, seed="+ str(seed())+")
        # fig.update_layout(
        #     legend=dict(
        #     yanchor="top",
        #     y=0.99,
        ...
        # )
        fig.update_layout(
            title="Moyenne empirique:  évolution en fonction du nombre de tirages sur plusieurs répétitions et <br> histogrammes après "+ str(n_samples)+" tirages" + " (loi de Bernoulli, seed="+ str(seed())+")",
