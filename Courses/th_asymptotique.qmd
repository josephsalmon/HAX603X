...
app_ui = ui.page_fluid(
    ...
    ui.layout_sidebar(
        ui.panel_sidebar(
            ...
            ui.input_slider( "p", "Paramètre de Bernoulli: p", 0.01, 0.99, value=0.5, step=0.01, ticks=False,
            ),
...
notons $\alpha=\mathbb{P} \left(\bar X_n \notin [ \mu + \tfrac{a \sigma}{\sqrt{n}},
...
\begin{align}
    1-\alpha& = \mathbb{P} \left(\bar X_n \in [ \mu + \tfrac{a \sigma}{\sqrt{n}}, \mu + \tfrac{ b \sigma}{\sqrt{n}}] \right)\nonumber\\
    & =
    \mathbb{P} \left(\tfrac{\bar X_n - \mu}{\sigma} \in [ \tfrac{a}{\sqrt{n}},\tfrac{b}{\sqrt{n}}]\right) \nonumber\\
    & =
...
Dans le cas classique d'un intervalle de confiance à 95\%, c'est-à-dire quand $\alpha=0.05$, et en prenant un intervalle de confiance symétrique (alors $a=-t$ et $b=q$) on obtient $1-\alpha= \int_{-q}^q \varphi(x) \,  dx=\Phi(q)-\Phi(-q)=2 \Phi(q)-1 \implies \boxed{q=\Phi^{-1}(1-\tfrac{\alpha}{2})}$ et $q$ est donc le quantile de niveau $1-\tfrac{\alpha}{2}$ de la loi normale centrée réduite. Numériquement on peut facilement évaluer $q$ et vérifier que $q\approx 1.96$ avec `scipy`:
