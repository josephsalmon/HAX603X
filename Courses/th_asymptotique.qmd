...
app_ui = ui.page_fluid(
    ...
    ui.layout_sidebar(
        ui.panel_sidebar(
            ...
            ui.input_slider( "n_samples", "Échantillons: n", 2, 1000, value=15, step=1),
          width=3
        ),
    ui.panel_main(output_widget("my_widget"), width = 12)
    )
)
...
$$
	...
	\underset{n \to \infty}{\longrightarrow}  \int_a^b \varphi(x) \, \mathrm dx\,.
$$
où l'on note $\varphi$ la densité d'une loi normale centrée réduite, définie pour tout $x\in\mathbb{R}$ par $\varphi(x)=\tfrac{e^{-\frac{x^2}{2}}}{\sqrt{2\pi}}$.
...
app_ui = ui.page_fluid(
    ui.panel_title("Théorème centrale limite"),
    ui.layout_sidebar(
        ui.panel_sidebar(
            ui.input_action_button("seed", "Nouveau tirage",class_="btn-primary"),
            ui.input_slider("p", "Espérance: p", 0.01, 0.99, value=0.5, step=0.01),
            ui.input_slider("n_repetitions", "Répétitions: t", 1, 300, value=200, step=1),
            ui.input_slider("n_samples", "Échantillons: n", 1, 200, value=100, step=1), width=3
        ),
    ui.panel_main(output_widget("my_widget"), width = 12)
    )
)
...
def server(input, output, session):
    ...
    def my_widget():
        ...
        fig = make_subplots(
                    ...
                    column_widths=[9, 1, 3],
                    subplot_titles=("Moyenne empirique:<br> n = " + str(n_samples) + " tirages", "        ", "Histogramme: <br>" + str(n_repetitions) + " répétitions")
                )
