...
proposa une méthode pour estimer la valeur de $\pi$ en 1733. Il s'agit de l'expérience de l'aiguille de Buffon. C'est l'une des premières méthodes de Monte-Carlo référencée dans la littérature (la source du texte est disponible sur le site de la BNF [ici](https://gallica.bnf.fr/ark:/12148/bpt6k10705425/f117.item)).

La question initiale (simplifiée ici) est la suivante:
...
La motivation initiale étant de faire jouer deux joueurs: l'un parie sur le fait que l'aiguille croise une ligne de la trame du parquet, l'autre sur le fait que l'aiguille ne croise pas une ligne de la trame du parquet. L'enjeu est alors de calculer la probabilité de succès de chacun des joueurs.

Voilà la question que s'est posée Buffon en 1733. La réponse est donnée par la formule suivante, qui montre que le jeu qu'il propose n'est pas équilibrer:

$$
P = \frac{2}{\pi} \approx 0.6366197723675814 \enspace.
$$
Une preuve de ce résultat sera donnée ci-dessous.

L'idée sous-jacente de Buffon est que l'on peut approcher cette quantité numériquement en proposant un estimateur $\hat{P}_n$ de cette quantité, consistant à répéter $n$ fois l'expérience du lancer.
Pour estimer $\pi$ il ne restera plus qu'à évaluer $\frac{2}{\hat{P}_n}$.

On peut faire cette expérience dans le monde réelle (c'est un peu long pour $n$ grand), mais on peut aussi utiliser une méthode numérique pour cela. On procède alors comme suit: il s'agit de tirer la position du centre de l'aiguille, puis de manière aléatoire son angle de chute, enfin de tester si l'aiguille croise une ligne de la trame du parquet.

Cette méthode est donnée ci-dessous, avec un exemple interactif.
...
En reprenant le même type de raisonnement que ci-dessus, trouver la distance entre les lattes du parquet qui rend le jeu équilibrer entre les deux joueurs introduit par Buffon (celui pariant sur un).
:::
