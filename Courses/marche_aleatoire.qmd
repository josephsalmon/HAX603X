...
Les marches aléatoires, comme tous les processus à valeurs discrètes, ont une représentation graphique naturelle. On représente graphiquement un tirage de la marche $(S_n)_{n\in\N}$ sur l'intervalle de temps $\llbracket 0,N \rrbracket$ par une ligne brisée joignant les points $(0,S_0)$, $(1,S_1)$, ...,$(N,S_N)$. On a donc en abscisse le temps, et en ordonnée la valeur de la marche.

::: {#exm-traj}
La trajectoire correspondant au tirage $S_0=0$, $X_1=1$, $X_2=1$, $X_3=-1$, $X_4=1$, $X_5=-1$ est donnée figure \ref{fig:chap03-marche}.
%=========================
\begin{figure}[htp]
\centering
\includegraphics[height=5cm]{figures/chap03-marche.pdf}
\caption{Représentation graphique d'une trajectoire de marche aléatoire \index{trajectoire de marche aléatoire} entre $0$ et $5$.\label{fig:chap03-marche}}
\end{figure}
%=========================
...
La figure \ref{fig:chap03-chemins} présente deux chemins de $(1,1)$ à $(5,3)$.
%=========================
\begin{figure}[htp]
\centering
\includegraphics[height=5cm]{figures/chap03-chemins.pdf}
\caption{Deux chemins de $(1,1)$ à $(5,3)$.\label{fig:chap03-chemins}}
\end{figure}
%=========================
:::
...
\ref{fig:chap03-reflex}.
%=========================
\begin{figure}[htp]
\centering
\includegraphics[height=5cm]{figures/chap03-reflex.pdf}
\caption{Un chemin de $(0,2)$ à $(11,1)$ passant par $0$ (en trait plein bleu) et son image (en pointillé rouge) par la symétrie de premier passage en $0$.\label{fig:chap03-reflex}}
\end{figure}
%=========================
...
\right.
\end{align*}
Cette application définit une bijection entre les deux ensembles de chemins. En effet, si on prend maintenant un chemin $(\wt S_m,\ldots,\wt S_n)$ de $(m,a)$ à $(n,-b)$, comme il fait des pas de $1$, que $a>0$ et $-b<0$, ce chemin passe nécéssairement par $0$. Soit $p$ le premier instant où le chemin est en $0$. On construit alors un chemin $(S_m,\ldots,S_n)$ de $(m,a)$ à $(n,b)$ passant par $0$ en posant
\begin{align*}
S_k=\left\{
\begin{array}{lcl}
...
Une première application de cette propriété est le résultat suivant connu sous le nom de *théorème su scrutin}.

::: {#thm-election}
Au cours d'une élection opposant deux candidates $A$ et $B$, la candidate $A$ a obtenu $a$ voix et la candidate $B$ $b$ voix, avec $a>b$. Alors la probabilité que $A$ ait été majoritaire (au sens large) tout au long du dépouillement est 
$$p=1-\frac{b}{a+1}.$$
:::
...
\begin{itemize}
\item au nombre de chemins de  $(0,0)$ à $(a+b,a-b)$ ne prenant aucune valeur strictement négative, autrement dit le nombre de chemins de $(0,0)$ à $(a+b,a-b)$ ne touchant pas $-1$;
\item au nombre de chemins de $(0,1)$ à $(a+b,a-b+1)$ ne touchant pas $0$, en décalant d'un cran les ordonnées;
\item au nombre total de chemins de $(0,1)$ à $(a+b,a-b+1)$ moins le nombre de chemins de $(0,1)$ à $(a+b,a-b+1)$ touchant $0$;
\item au nombre total de chemins de $(0,1)$ à $(a+b,a-b+1)$ moins le nombre de chemins de $(0,1)$ à $(a+b,-(a-b+1))$ par le principe de réflexion.
\end{itemize}
...
\begin{align*}
{\P(T_0=2n)}\\
&=\P(S_1=1, S_{2n-1}=1)-\P(S_1=1, S_{2n-1}=-1)\\
&=\P(S_{2n-1}=1|S_1=1)\P(S_1=1)-\P(S_{2n-1}=-1|S_1=1)\P(S_1=1)\\
...
\begin{align*}
{\P(S_{2n}=0)}  \\
 &=\P(S_{2n}=0, S_{2n-2} =-2)+\P(S_{2n}=0, S_{2n-2}=0) +\P(S_{2n}=0, S_{2n-2}=2)\\
&=2\P(S_{2n}=0, S_{2n-2}=-2)+\P(S_{2n}=0, S_{2n-2}=0),
\end{align*}
par symétrie. Il vient
\begin{align*}
{\P(S_{2n}=0)}\\
&=2\P(S_{2n}=0|S_{2n-2}=-2)\P(S_{2n-2}=-2)+\P(S_{2n}=0|S_{2n-2}=0)\P(S_{2n-2}=0)\\
&=2\P(S_2=2|S_0=0)\P(S_{2n-2}=-2)+\P(S_2=0|S_0=0)\P(S_{2n-2}=0)\\
&=2\frac{1}{4}\P(S_{2n-2}=-2)+\frac{1}{2}\P(S_{2n-2}=0)\\
&=\frac{1}{2}\P(S_{2n-2}=-2)+\frac{1}{2}\P(S_{2n-2}=0),
\end{align*}
où on a utilisé le fait que
\begin{align*}
 \P(S_2=2|S_0=0)& =   \P(X_1 + X_2 = 0) \\
...
\begin{itemize}
\item au nombre de chemins de $(0,0)$ à $(n-1,a-1)$ ne touchant pas $a$
\item au nombre de chemins de $(0,0)$ à $(n-1,a-1)$ - le nombre de chemins de $(0,0)$ à $(n-1,a-1)$ touchant $a$
\item au nombre de chemins de $(0,0)$ à $(n-1,a-1)$ - le nombre de chemins de $(0,-a)$ à $(n-1,-1)$ touchant $0$
\item au nombre de chemins de $(0,0)$ à $(n-1,a-1)$ - le nombre de chemins de $(0,-a)$ à $(n-1,1)$
\item au nombre de chemins de $(0,0)$ à $(n-1,a-1)$ - le nombre de chemins de $(0,0)$ à $(n-1,a+1)$
\end{itemize}
Ainsi, on a  
\begin{align*}
\P(T_a=n)
...
::: {#prp-donsk}
Pour tous $t_0=0<t_1 <...< t_p$, le vecteur $(B^N_{t_1},...,B^N_{t_p})$ converge en loi vers un vecteur $(B_{t_1},...,B_{t_p})$ tel que pour tout $i \in\{1,...,p\}$, $B_{t_{i}}-B_{t_{i-1}}$ est indépendant de $B_{t_{i-1}}$ et suit une loi normale $\mathcal{N}(0, t_i - t_{i-1} )$
:::
...
$$
donc les accroissements du vecteur $(B^N_{t_1},...,B^N_{t_p})$ sont indépendant. Il suffit maintenant de montrer qu'ils convergent vers la loi normale.  On a
$$
