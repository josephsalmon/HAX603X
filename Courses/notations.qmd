...
def cdf_tool(x):
    y = np.zeros(2*(len(x)), dtype='int64')
    y[::2]=x
    y[1::2]=x
    return y[1::], y[:-1]
...
def server(input, output, session):
    ...
    def my_widget():
        ...
        # Quantile plot
        new_x, new_y = cdf_tool(support)

        fig.add_trace(
            go.Scatter(
                x=insert_nones(list(cdf_data[new_x])),
                y=insert_nones(list(x[new_y])),
                mode="lines",
        ...
        fig.add_trace(
            go.Scatter(
                x=insert_nones(list(x[new_x])),
                y=insert_nones(list(cdf_data[new_y])),
                mode="lines",
                line=dict(color="black")
            ),
            row=2,
