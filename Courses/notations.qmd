...
{
  const x = d3.range(-5, 5, 0.01);
  const pmf = x.map(x => dists[distrib_name_discr].pmf(x, mu_disc, sigma_disc));
  const cdf = x.map(x => dists[distrib_name_discr].cdf(x, mu_disc, sigma_disc));
  const inv = x.map(x => dists[distrib_name_discr].quantile(x, mu_disc, sigma_disc));
  const quantile = dists[distrib_name_discr].quantile(alpha_disc, mu_disc, sigma_disc);
  const filteredX = x.filter(coord => coord <= quantile);
  const filteredPmf = pmf.filter((_, i) => x[i] <= quantile);
  const filteredCdf = cdf.filter(coord => coord <= quantile);
