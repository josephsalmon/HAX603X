...
donnée par
$$
f(x) = \gamma e^{-\gamma x} \1_{\mathbb{R}_+}(x)\enspace.
$$
...
$$
f(x) = \frac{1}{{(2 \pi)}^{\frac{d}{2}} {\mathrm{det}}(\Sigma)} e^{-\frac{1}{2}(x-\mu)^\top \Sigma ^{-1}(x-\mu)}
$$
...
$$
    F_X(x) = \sum_{i \in I} p_i \1_{[x_i, \infty[}(x) \enspace.
$$
...
##  Fonction quantile/ inverse généralisée :gb:: *quantile distribution function*
...
## Note sur le calcul des quantiles

Concernant le cas gaussien, `scipy` utilise par exemple la fonction spéciale `ndtri`, voir par exemple le fichier source de [scipy](https://github.com/scipy/scipy/blob/df134eab5a500c2146ed4552c8674a78d8154ee9/scipy/stats/_continuous_distns.py#L376), pour calculer les quantiles avec  `norm.ppf`. Plus en détails, la fonction `ndtri` est une fonction de la librairie `cephes` qui est une librairie de fonctions mathématiques écrites en C, et utilise des interpolations et l'algorithme de Horner pour calculer des approximations par fractions rationnelles de la fonction; le code est dipsonible ici: [ndtri.h](https://github.com/scipy/scipy/blob/e3c6a053cdfb13932cd6405f01a98821dce41e80/scipy/special/xsf/cephes/ndtri.h).
...
viewof inputs = Inputs.form([
      Inputs.range([-10, 10], {value: 0.1, step: 0.001, label: tex`Param 1`}),
      Inputs.range([0.01, 5], {value: 1.01, step: 0.001, label: tex`Param 2`}),
      Inputs.range([0.001, 0.999], {value: 0.75, step: 0.001, label:tex`\alpha (quantile)`}),
    ]);
