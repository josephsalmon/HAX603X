...
// dists = x.map(e=>stdlib.base.dists.cauchy.pdf( e, 2, 1))
// continuous case 
jstatPDFs = () => {
  const distributions = Object.keys(stdlib.base.dists);
  print(distributions)
  // Get in continuousDistributions the distributions whose pdf, cdf and quantile are defined
    const continuousDistributions = distributions.filter(name => stdlib.base.dists[name].pdf && stdlib.base.dists[name].cdf && stdlib.base.dists[name].quantile);
//   const continuousDistributions = distributions.filter(name => distributions[name].pdf && distributions[name].cdf && distributions[name].quantile);
//   const pdfs = {};
//   const cdfs = {};
//   const invs = {};
...
//         // }
//     // else{
//     pdfs[name] = distributions[name].pdf;
//     cdfs[name] = distributions[name].cdf;
//     invs[name] = distributions[name].quantile;
//     // }
//   }
...

{
  ...
  const pdf = x.map(x => stdlib.base.dists[distrib_name].pdf(x, mu, sigma));
  const cdf = x.map(x => stdlib.base.dists[distrib_name].cdf(x, mu, sigma));
  const inv = x.map(x => stdlib.base.dists[distrib_name].quantile(x, mu, sigma));
  const quantile = stdlib.base.dists[distrib_name].quantile(alpha, mu, sigma);
