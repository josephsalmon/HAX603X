...
$$
f(x) = \frac{1}{{(2 \pi)}^{\frac{d}{2}} {\rm det}(\Sigma)} e^{-(x-\mu)^\top \frac{\Sigma ^{-1}}{2}(x-\mu)}
$$
...
def server(input, output, session):
    ...
    def my_widget():
        fig = make_subplots(
            ...
            column_widths=[3.5, 6.5],
            row_heights=[7 / 13, 4 / 13, 2 / 13],
        )
        ...
        fig.update_layout(autosize=False, height=715, width=550)
