...
\begin{align}
   ...
   \{x \in \mathbb{R} : x \geq F_X^\leftarrow(q)  \}
\end{align}
$$ 
:::
...
- Cas $\supset$: Soit  $x \in \mathbb{R}$ t.q. $x \geq F_X^\leftarrow(q)$ alors pour tout $\epsilon > 0$, $x + \epsilon > F_X^\leftarrow(q)$, donc $F(x + \epsilon) \geq q$. Puis, par continuité à droite de $F$, $F(x) \geq q$.
