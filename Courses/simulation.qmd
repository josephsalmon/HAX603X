...
Pour être plus rigoureux, ces nombres sont en fait des nombres entiers générés uniformément sur un certain intervalle. Dans un second temps, une transformation simple (normalisation) permet d'obtenir des nombres flottants (&#127468;&#127463;: *floats*) entre 0 et 1.
:::
...
- Cas $\supset$: Soit  $x \in \mathbb{R}$ t.q. $x \geq F^\leftarrow(q)$ alors pour tout $\epsilon > 0$, $x + \epsilon > F^\leftarrow(q)$, donc $F(x + \epsilon) \geq q$. Puis, par continuité à droite de $F$, $F(x) \geq q$.
...
## Méthode d'inversion
Soit $X$ une v.a réelle, et  $U \sim\mathcal{U}([0,1])$, alors la variable aléatoire $F_X^{}(U)$ a même loi que $X$.
:::


[Preuve]{.underline}: En utilisant le théorème précédent, on a
$\mathbb{P}(F_X^{-1}(U) \leq x) = \mathbb{P}(U \leq F_X(x))$ pour tout $x\in\mathbb{R}$.
...
fig1.show()
```
Nous pouvons facilement améliorer la proportion de point acceptés en proposant par exemple $g$ définie par $g(x) = \tfrac{x}{2} {1\hspace{-3.8pt} 1}_{[0, 1]}(x)$, et $m=8$.
...
On peut désormais prouver le résultat de la Proposition @{prp-poisson_exponentielle}.
