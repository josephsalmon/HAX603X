...
for i, name_distrib in enumerate(['norm', 'poisson', 'geom']):
    ...
    fig.add_trace(go.Scatter(x=[q_alpha], y=[alpha], mode="markers", marker=dict(color=color_blue, symbol="circle", size=15), name="Quantile", showlegend=False), row=1,col=i+1)

    fig.add_trace(go.Scatter(x=[x[0] + x.ptp() / 10], y=[alpha + 0.03], text=r"u", mode="text", showlegend=False), row=1,col=i+1)
    
    fig.add_trace(go.Scatter(x=[q_alpha + 0.1], y=[0.03], text=r"$c", mode="text", showlegend=False), row=1,col=i+1)
        # Axes ranges
