...
Pour être plus rigoureux, ces nombres sont en fait des nombres entiers générés uniformément sur un certain interval. Dans un second temps, une transformation simple (normalisation) permet d'obtenir des nombres flottants (:gb:: *floats*) entre 0 et 1.
:::
...
1. On part d'une graine (:gb:: *seed*) $U_0$ qui détermine la première valeur de manière la plus arbitraire possible.
...
La plupart des PRNG s'appuient sur des résultats arithmétiques. Un des plus connus est celui appelé Générateur congruentiel linéaire (:gb: Linear congruential generator, LCG).
...
Rappel : Pour $F$ une fonction définie sur $\mathbb{R}$ à valeurs dans $[0, 1]$, croissante, on note $\forall q \in ]0,1[, $

$$
F^\leftarrow(q) = \inf\{ x \in \mathbb{R} : F(x)\geq q\}
$${#eq-inf}
...
Soit $F$ une fonction définie sur $\mathbb{R}$ à valeurs dans $[0, 1]$, croissante et continue à droite, alors pour tout $q \in ]0, 1[$, $x \in \mathbb{R}$ on a

\begin{align*}
  F(x) \geq q \iff  x \geq F^\leftarrow(q) \enspace.
\end{align*}
:::
...
$$
f_{\lambda}(x) = \lambda e^{-\lambda x} \1_{\mathbb{R}_+}(x)\enspace.
$$
et donc pour fonction de répartition
$$
F_{\lambda}(x) = (1 - e^{-\lambda x}) \1_{\mathbb{R}_+}(x)\enspace.
$$
...
$$
F(x) = (1 - e^{-(x/\lambda)^k})\1_{\mathbb{R}_+}(x)
$$
...
#| fig-cap:
#|       - "Fonction de répartition et quantiles: solution de F(x) = u"
...
$$
    ...
    \end{array}
$$
ce qui se réécrit plus simplement $F^\leftarrow(u) = \1_{\{1-p < u\}}$.

Ainsi, si $U$ suit une loi uniforme sur $[0,1]$ alors $\1_{\{1-p < U\}}$ suit une loi de Bernoulli de paramètre $p$.
Comme $1-U$ suit aussi une loi uniforme sur $[0,1]$, on en déduit que $\1_{\{U < p\}}$ suit une loi de Bernoulli de paramètre $p$.
...
$$
		F^\leftarrow(u) = \sum_{k=1}^r x_k \1_{ \{  \sum_{i=1}^{k-1}p_i < u \leq \sum_{i=1}^{k}p_i \} }\enspace,
$$
où on a posé $p_0=0$.
:::
...
La méthode est illustrée ci-dessous pour quelques lois intéressantes:

::: {.content-visible when-format="html"}

```{ojs}
//| echo: false
viewof dist = Inputs.select(['normal','cauchy', 'exponential','laplace','bimodal', 'geometric', 'uniform'], {value: "bimodal", label: "Loi"})
...
// the repo is
// invcdfboard = {
//   const response = await fetch("https://raw.githubusercontent.com/fradav/matter-playground/refs/heads/gh-pages/invcdfboard.umd.cjs");
...
- on sait évaluer le **rapport d'acceptation** $r(x) = \frac{f(x)}{mg(x)}$, sur le support de $g$ (qui contient celui de $f$).
...
$$
    \mathbb{P}(X \leq x, T=n) =  a^{n-1} b\,,
$$
avec $a=\mathbb{P}(U_1 > r(Y_1))$ et $b=\mathbb{P}(U_n \leq r(Y_n), Y_n \leq x)$.
Concernant le premier terme, les variables aléatoires $Y_1$ et $U_1$ sont indépendantes donc leur loi jointe correspond au produit des densités, ainsi:
$
\begin{align*}
	   a & = \mathbb{P}(U_1 > r(Y_1))\\
		 & = \mathbb{P}((U_1, Y_1) \in \{(u,y) \in \mathbb{R}^2 : u > r(y)\})                             \\
		 & = \int_{\mathbb{R}^2} \1_{\{u > r(y)\}} (\1_{[0,1]}(u) g(y)) \, du dy  \\
		 & = \int_{\mathbb{R}} \bigg( \int_0^1 \1_{\{u > r(y)\}} \, du\bigg) g(y)\, d y \\
...
\begin{align*}
      b & = \mathbb{P}(U_n \leq r(Y_n), Y_n \leq x)\\
        & = \int_{\mathbb{R}^2} \1_{\{u \leq r(y)\}} \1_{\{y \leq x\}} (\1_{[0,1]}(u) g(y)) \, du dy       \\
        & = \int_{\mathbb{R}} \bigg( \int_0^1  \1_{\{u \leq r(y)\}} \, du\bigg) \1_{\{y \leq x\}}  g(y)\, d y \\
		& = \int_{\mathbb{R}} r(y) \1_{\{y \leq x\}}  g(y)\, d y \\
...
Donnons un exemple jouet (on étudiera des exemples plus pertinents en TD). On considère la densité $f(x) = 4x^3 \1_{[0,1]}(x)$. Comme $f$ est majorée par $4$, on peut choisir pour $g$ la densité de la loi uniforme sur $[0,1]$ et $m=4$. Alors, $r(x) =f(x) / (mg(x)) = x^3$, pour $x \in [0,1]$. On simule donc $(Y_1, U_1)$ et on teste si $U_1 \leq Y_1^3$, etc.
...
fig1.write_image("../srcimages/rejet_polynome_unif.svg")
fig1.show()
...
![](../srcimages/rejet_polynome_unif.svg)

:::

Nous pouvons facilement améliorer la proportion de point acceptés en proposant par exemple $g$ définie par $g(x) = 2x \1_{[0, 1]}(x)$, et $m=2$.
...
fig2.write_image("../srcimages/rejet_polynome_tri.svg")
fig2.show()
```
:::
...
![](../srcimages/rejet_polynome_tri.svg)
...
Considérons la densité d'Andrews définie par $f(x) = \frac{1}{S} \frac{\sin(\pi\cdot x)}{\pi \cdot x}  \1_{[-1,1]}(x)$, avec $S = \int_{-1}^{1}\frac{\sin(\pi\cdot x)}{\pi \cdot x}dx$.
...
si l'on prend $m=2/S$ et $g(x) = \frac{1}{2} \1_{[-1,1]}(x)$, on observe que pour $x\in [-1,1]$, tester $u\leq \frac{f(x)}{m \cdot g(x)}$ est équivalent à tester $u \leq r(x)=\frac{1}{S} \frac{\sin(\pi\cdot x)}{\pi \cdot x}  \cdot \frac{1}{\frac{2}{S} g(x)} = \frac{\sin(\pi\cdot x)}{\pi \cdot x}  \cdot \frac{1}{2 \cdot g(x)}$, ce qui peut se faire sans connaissance de $S$.
De plus on peut vérifier que $g(x) = \frac{1}{2} \1_{[-1,1]}(x)$ définit une densité et que $f(x) \leq m \cdot g(x)$ pour tout $x\in \mathbb{R}$.
...
fig.update_layout(template="simple_white", showlegend=True)
fig.write_image("../srcimages/rejet_andrews.svg")
fig.show()
```
:::
...
fig.show()
fig.write_image("../srcimages/rejet_cercle.svg")
...
fig = plot_curve(fig, plot_type=plot_type, n_discr=100)
fig.show()
fig.write_image("../srcimages/rejet_cardioide.svg")
...
Pour simuler des variables aléatoires de Bernoulli, on utilise la méthode d'inversion (voir @exm-inverse-bernoulli). Ainsi, si $U_1, \ldots, U_n$ sont des variables aléatoires iid de loi uniforme sur $[0,1]$, alors
$$
	\sum_{i=1}^n \1_{\{U_i \leq p\}} \sim \mathcal{B}(n,p)\,.
$$
...
On montre le résultat pour $n=2$, et on note $q_n =     \mathbb{P}(E_1+\dots+E_n \leq t)$.
La généralisation à $k$ quelconque se fait par récurrence. 
Soit $t > 0$, et $f_{\lambda}(x)=\1_{\{x \geq 0 \}} \lambda e^{-\lambda x}$ la densité d'une loi exponentielle de paramètre $\lambda$. Les variables aléatoires $E_1$ et $E_2$ étant indépendantes et suivant des lois exponentielles de paramètre $\lambda_1$ et $\lambda_2$, on a

\begin{align*}
    q_2 & = \mathbb{P}(E_1+E_2 \leq t)\\
        & = \int_{\mathbb{R}^2} \1_{\{x_1 + x_2 \leq t\}} f_{\lambda}(x_1) f_{\lambda}(x_2)\, d x_1 d x_2 \\
        & = \int_{\mathbb{R}^2} \1_{\{x_1 + x_2 \leq t, x_1 \geq 0, x_2 \geq 0\}}\lambda^2 e^{-\lambda (x_1+x_2)} \, d x_1 d x_2 \\
        & = \int_{\mathbb{R}^2} \1_{\{0 \leq x_1 \leq t, 0 \leq x_2 \leq t-x_1\}} \lambda^2 e^{-\lambda x_1} e^{-\lambda x_2}\, d x_1 d x_2             \\
...
::: {.proof}
Notons d'abor que pour tout $n \in \mathbb{N}$
$$
    \mathbb{P}(T=n) = \mathbb{P}(S_n \leq 1 < S_{n+1})\,.
$$

On décompose donc la probabilité $\mathbb{P}(S_n \leq 1 < S_{n+1})$ via

\begin{align*}
    \mathbb{P}(T=n)
    & = \mathbb{P}(S_n \leq 1 < S_{n+1})\\
    & = \mathbb{P}(\{S_n \leq 1\} \mathbin{\backslash} \{S_{n+1} \leq 1\})\\
