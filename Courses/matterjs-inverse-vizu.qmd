...
Matter = require(await FileAttachment("./matter.alpha.min.js").url()).catch(() => window["Matter"])
{
    ...
    Example.galton = function() {
        ...
        // add bodies
        const distribution = 'exponential';
        let total = 500;
        setInterval(() => {
            if (total-- > 0) {
            ...
            const square = Bodies.rectangle(discretize((quantile*40+width/2) ), 1*height/3 + (-(unifm1p1-0.5)*height/3)
            , size,size, {
                sleepThreshold: 10,
                friction: 1,
                density: 1,
        ...
        World.add(
            world,
            Bodies.rectangle(width/2, height-10, width, 4, {
            isStatic: true,
            render: {
                fillStyle: "#ffffff",
                visible: true
