...
Matter = require(await FileAttachment("./matter.alpha.min.js").url()).catch(() => window["Matter"])
{
    ...
    Example.galton = function() {
        ...
        setInterval(() => {
            ...
            const quantile = dists['laplace'].quantile(unifm1p1, mu, sigma);
            const square = Bodies.rectangle(discretize((-quantile*40+width/2) ), height/2 + ((unifm1p1-0.5)*200)
            , size,size, {
                sleepThreshold: 10,
                friction: 1,
                restitution: 0.0,
        ...
        World.add(
            world,
            Bodies.rectangle(width/2, height-2, width, 4, {
            isStatic: true,
            render: {
                fillStyle: "#ffffff",
                visible: true
